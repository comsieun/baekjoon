...
public class main {
	...
	public static int F(int s, int r, int c) {
		if(r==0&&c==0) return 0;
			...
			if(r<s/2 && c<s/2) return F(s/2, r, c);
			if(r<s/2 && c>=s/2) return (s*s/4)+F(s/2, r, c-(s/2));
			if(r>=s/2 && c<s/2) return (s*s/4)*2+F(s/2, r-(s/2), c);
			if(r>=s/2 && c>=s/2) return (s*s/4)*3+F(s/2, r-(s/2), c-(s/2));
		}
		return c;
	}
}